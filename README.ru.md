...
- **Меньше контекста**: инструмент возвращает `markdown_path`, а не весь текст документа.
- **Локальная обработка**: для извлечения и OCR не нужны внешние API-ключи.
- **Удобно для агента**: tools регистрируются в существующий Hermes toolset `file`.
- **Повторяемость**: повторное использование кэша по хэшу файла и настройкам извлечения.
- **С учётом приватности**: `sensitive=true` скрывает source paths и отключает previews по умолчанию.

## Для кого этот плагин

Используйте этот плагин, если хотите, чтобы Hermes Agent работал с локальными PDF, сканами, скриншотами, Office-документами и изображениями без загрузки документов целиком в контекст модели.

Хорошо подходит для:

- `llm-wiki` и knowledge-base workflows
- локального анализа документов
- PDF-to-Markdown pipelines
- OCR для скриншотов и сканированных страниц
- agent workflows, где важен расход контекста
